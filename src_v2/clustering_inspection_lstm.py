...
def outer_clustering_analysis(data, gt_clu, 
    ...
    for j in range(0, max(clusters) + 1):
        ...
        for idx, p in enumerate(pu):
            print("P={0:.2f} in cluster disease {1}".format(p, idxd[idx]))
            if p == max_pu:
                print("max(P) in cluster disease {0}".format(idxd[idx]))
...
def clustering_inspection(indir,
    ...
    with open(path.join(expdir, 'encoded-umap_vect.csv'), 'w') as f:
        wr = csv.writer(f)
        for eu in encoded_umap:
            wr.writerow(eu)
    ...
    colors_en1 = [colors_vec[disease_dict[v]] for v in disease_class_first]
    #colors_en1 = [colormap[disease_dict[v]] for v in disease_class_first]
    single_plot(encoded_umap, disease_class_first, colors_en1, 
                path.join(expdir, 'cnn-ae_encodings_plot.eps'))
    # plot cluster results
    clusters, pur_vec, idxd = outer_clustering_analysis(encoded, disease_class_first, HCpar['linkage_clu'], HCpar['affinity_clu'], preproc=True)
    with open(path.join(expdir, 'purity_vector.csv'), 'w') as f:
        wr = csv.writer(f)
        for p in pur_vec:
            wr.writerow(p)
    ...
    for p in pur_vec:
        idx = p.index(max(p))
        labels.append(idxd[idx])
    colors_en2 = [colors_vec[v] for v in clusters]
    #colors_en2 = [colormap[v] for v in clusters]
    single_plot(encoded_umap, clusters, colors_en2, 
                path.join(expdir, 'cnn-ae_outer-clust_plot.eps'), labels)
    # histogram of purity measures
    n_groups = 6
    ...
    for idx in range(n_groups):
        rects = plt.bar(index, [p[idx] for p in pur_vec], bar_width,
        alpha=opacity,
        color=colors_vec[idx],
        label=idxd[idx])
        index = index + bar_width

    plt.xlabel('')
    plt.ylabel('Purity score', size=15)
    plt.title('')
    plt.xticks(np.arange(n_groups) + 0.375, 
              ('Clust:0', 'Clust:1', 'Clust:2', 'Clust:3', 'Clust:4', 'Clust:5'), size=15)
    plt.yticks(size=15)
    plt.legend(markerscale=5, fontsize=15)
    plt.savefig(path.join(expdir, 'hist-encoded-purity.eps'), format='eps', dpi=1000)
 ...
 #               path.join(expdir, 'count_encodings_plot.eps'))
    # plot cluster results
 #   clusters, _, _ = outer_clustering_analysis(svd_mat, raw_disease_class_first, HCpar['linkage_clu'], 
 #                                        HCpar['affinity_clu'], preproc=False)
 #   for c in set(clusters):
 #       print("Cluster {0} numerosity = {1}".format(c, clusters.count(c)))
 ...
 #               path.join(expdir, 'count_outer-clust_plot.eps'))
 ...
 #               path.join(expdir, 'count_sub-clust_plot.eps'))
 ...
 #   fig.savefig(path.join(expdir, 'silhouette_plot.eps'), format='eps', dpi=1000)
