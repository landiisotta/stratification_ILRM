            ...
            for r in rd:
                ...
                if len(seq) < len_padded:
                    for s in seq:
                        if s not in seq_tmp:
                            seq_tmp.append(s)
                    self.ehr[r[0]] = seq_tmp + [0] * (len_padded - len(seq_tmp))
                else:
                    for i in range(0, len(seq), len_padded):
                        l = []
                        for s in seq[i:i+len_padded]:
                            if s not in l:
                                l.append(s)
                        seq_tmp.extend(l)
                    if len(seq_tmp) % len_padded != 0:
                        nseq, nleft = divmod(len(seq_tmp), len_padded)
                        self.ehr[r[0]] = seq_tmp + [0] * \
                            (len_padded - nleft - (seq_overlap - 1) * nseq)

                    else:
                        self.ehr[r[0]] = seq_tmp
